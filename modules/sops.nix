...
# (config.users.users.${config.fleet.adminUser}.home) rather than a hardcoded path, so the module
# is not username/home-locked.
{ config, ... }:
{
  sops.defaultSopsFile = ../secrets/secrets.yaml;
  sops.age.keyFile = "${config.users.users.${config.fleet.adminUser}.home}/.config/sops/age/keys.txt";
