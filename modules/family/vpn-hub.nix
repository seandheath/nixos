...
let
  ...
  # Delete-then-append so a firewall reload cannot stack duplicates. NixOS runs
  # extraCommands on both start and reload; -D is a no-op the first time.
  addRules = lib.concatMapStringsSep "\n" (r: ''
    iptables -D FORWARD ${r} 2>/dev/null || true
    iptables -A FORWARD ${r}
  '') forwardRules;

  delRules = lib.concatMapStringsSep "\n" (r: ''
    iptables -D FORWARD ${r} 2>/dev/null || true
  '') forwardRules;
in
{
