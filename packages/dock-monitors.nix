...
let
  ...
  script = pkgs.writeText "dock-monitors.py" ''
import dbus
import sys
...
in {
  inherit pythonWithDbus script;
  package = pkgs.writeShellScriptBin "dock-monitors" ''
    exec ${pythonWithDbus}/bin/python3 ${script} "$@"
  '';
}
